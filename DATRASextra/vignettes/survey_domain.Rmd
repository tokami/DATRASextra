...
## requires also: remotes::install_github("ropensci/rnaturalearthhires")
```


## *Download DATRAS, read-in, clean, and get unique trawling events*

We start by extracting unique trawl haul locations from the DATRAS data and converting them into spatial points. We use the [*BITS*](https://gis.ices.dk/geonetwork/srv/api/records/f2f6f003-4dec-41c2-95d1-29dd48876662) survey as an example.

```{r, fig.alt = "",results='hide'}
survey <- "BITS"
tmp <- tempdir()
downloadDATRAS(surveys = survey, dir = tmp, years = c(2019:2021))
bits <- readDATRAS(file.path(tmp, survey)) # avoid printing
bits <- clean(bits)
# Extract unique trawls (haul id, lon, lat, Depth)
unique_trawls <- unique(bits[["HH"]][, c("haul.id", "lon", "lat")])
