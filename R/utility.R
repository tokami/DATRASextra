...
##'
##' @keywords internal
##' @importFrom graphics abline axis barplot box hist image layout legend lines mtext par plot.new points polygon
##' @importFrom grDevices grey
##' @importFrom stats approx coefficients lm median na.omit predict quantile qt reshape setNames var weighted.mean xtabs
##' @importFrom utils capture.output data getFromNamespace setTxtProgressBar txtProgressBar write.table
"_PACKAGE"

utils::globalVariables(c(
  "DataType", "Depth", "Gear", "HaulVal", "IndWgt", "Quarter",
  "SpecVal", "StdSpecRecCode", "Survey", "Valid_Aphia", "Year",
  "haul.id", "species_info"
))
...
##' @param resolution Step size between grid nodes, in the same units as
##'   \code{x} and \code{y}.
##' @param max_dist Optional distance threshold in the same units as
##'   \code{x} and \code{y}.  Grid nodes whose nearest observation is
##'   farther than \code{max_dist} are dropped.  Uses a k-d tree via
##'   \pkg{RANN} for efficiency.
...
make_survey_grid <- function(x, y, resolution, max_dist = NULL, time = NULL) {
  ...
  if (!is.null(time)) {
    grid <- merge(grid, data.frame(year = time), by = NULL)
  }
