...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Processing ICES DATRAS Data for FishGlob}
...
```
an example dataset containing `r length(unique(mini_fishglob$HL$Species))` species
across `r length(unique(mini_fishglob$HH$Survey))` surveys
(`r paste(sort(unique(mini_fishglob$HH$Survey)), collapse = ", ")`):

`r paste(sort(unique(mini_fishglob$HL$Species)), collapse = ", ")`
...
The `add_total_weight_by_haul_fishglob()` function converts length data to
weight using species-specific **length–weight relationships**.
